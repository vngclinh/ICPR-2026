...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-sr-cache",
        action="store_true",
        help="Load cached SR frames (sr-*.png) instead of lr-*.png. Generated by gen_sr_images.py.",
    )
    parser.add_argument(
        "--sr-prefix", type=str, default="sr",
        help="Filename prefix for cached SR frames (default: 'sr').",
    )
...
def build_test_dataset(config: Config, use_sr_cache: bool = False, sr_prefix: str = "sr") -> Tuple[Optional[MultiFrameDataset], bool]:
...
def main() -> None:
    ...
    common_ds_params = {
        ...
        "use_sr_cache": args.use_sr_cache,
    ...
    test_ds, test_is_labelled = build_test_dataset(
        config, use_sr_cache=args.use_sr_cache, sr_prefix=args.sr_prefix,
    )
